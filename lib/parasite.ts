export var p$ = (function () {
    ...
    const WIN: WIN = window || DOC.defaultView; 
    ...
    pjs.findName = function (name) {
        let collection: any = DOC.getElementsByName(name), parent;

        if (this === DOC || !collection.length) { return collection; }
    ...
    function buildParam(obj, prefix, forceString?: boolean): string {
     ...
     */
    function isType(obj: any, types: string | string[]): boolean {
        let match = Array.isArray(obj) ? 'array' : (typeof obj),
            some = (type) => {
     ...
     */
    function parseHTML(htmlString: string): ParasitedList {
     ...
     */
    function require(file: string): void {
        const script = DOC.createElement('script');
        script.src = file;
    ...
    function get(urlOrOptions, dataOrSuccess?, success?): Deferred {
        return requestBuilder('GET', urlOrOptions, dataOrSuccess, success);
    }

    function post(urlOrOptions, dataOrSuccess?, success?): Deferred {
    ...
    function ajax(url, options:any = {}): Deferred {
    ...
    class Deferred {
        ...
        private changeState(newState: string, context, args): boolean {
    ...
    function setFn(fnName: string, classes: any[], fn: Function = pjs[fnName]) {
        if (!fn) {
    ...
    function setAcessorFn(fnName: string, classes) {
    ...
    function eachTokens(rawTokens: string, iterator: (token: string) => any): void {
        rawTokens.split(' ').forEach(iterator);
    }

    function someToken(rawTokens: string, iterator: (token: string) => any): boolean {
        return rawTokens.split(' ').some(iterator);
    }

    /* =============== GLOBAL =============== */

    const p$: any = (selector) => {
        if (isType(selector, 'function')) { return ready(selector); }
        
        if (isType(selector, ['document', 'window'])) { return selector; }
